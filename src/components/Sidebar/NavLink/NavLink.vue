<template>
  ...
  >
    <router-link :to="link" class="sidebar-link">
      <!-- 图标，用的是flaticon的图标，相当于一个私有库了，class也是flaticon自定义的 -->
      <span class="icon">
    ...
    <div @click="() => togglePanelCollapse(link)">
      ...
      <router-link :to="link" event="" class="d-flex sidebar-link">
        ...
        <sup
          ...
          >{{ label }}</sup
        >
        <!-- 右向箭头，表示可以点击下拉，carretActive是让i标签旋转90度的类 -->
...
export default {
  ...
  methods: {
    // 'changeSidebarActive', // 将 `this.changeSidebarActive()` 映射为 `this.$store.dispatch('changeSidebarActive')`
    // 使用起来更简洁
